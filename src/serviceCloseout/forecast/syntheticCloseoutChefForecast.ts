...
const SYNTHETIC_FORECAST_ACTOR_ID = 'synthetic-test-actor';
const SYNTHETIC_FORECAST_ACTOR_NAME = 'Synthetic Test Chef';
...
function buildSyntheticChefForecastActivity(serviceDate: string): SyntheticChefForecastActivity {
  ...
  return {
    ...
    actor: {
      id: SYNTHETIC_FORECAST_ACTOR_ID,
      name: SYNTHETIC_FORECAST_ACTOR_NAME,
      image: null,
    },
 ...
 */
export function buildSyntheticCloseoutChefForecast(serviceDate: string): GameBusChefForecast {
  const parsed = parseGameBusChefForecastActivity(buildSyntheticChefForecastActivity(serviceDate));
  if (!parsed.ok) {
