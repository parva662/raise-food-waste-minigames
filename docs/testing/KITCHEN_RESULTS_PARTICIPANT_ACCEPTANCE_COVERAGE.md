...
1. Historical Progress is independent of the current waiting / no-forecast state.
2. Progress chart: 0 → empty; 1 → show data point (no trend claim); 2+ → chart + trend.
3. Other-staff comparison from **1** peer; label "Other staff" (1) / "Other staff median" (2+).
4. Missing forecast ≠ zero performance; omit from aggregation.
5. Closeout without personal forecast still shows actual kitchen outcome.
6. Forecast without closeout preserves structure + shows submitted forecast; metrics pending.
7. Shared calculation engine with admin for the same actor + targetDate.
8. No composite score / ranking / winner language.
...
| Default results service date | Exact product default when current date incomplete | — | `@pending` in Gherkin |
